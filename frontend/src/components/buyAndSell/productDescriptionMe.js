...
import UnAuthorizedPrompt from "./UnauthorizedPrompt";

const ProductDescriptionMe = (props) => {
  ...
  const [unauthorizedPrompt, setUnauthorizedPrompt] = useState(false);
    ...
    const getProduct = async () => {
      ...
      try {
        ...
        if (res.status === 200 && res.data.success) {
          setProductData(res.data.product.length > 0 ? res.data : {});
          setLoader(false);
        }
        else if(res.status === 200 && !res.data.success){
          setUnauthorizedPrompt(true);
          setLoader(false);
        }
      } catch (err) {
        console.log(err);
...
export default ProductDescriptionMe;
